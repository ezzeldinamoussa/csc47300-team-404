...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function toggleTaskState(taskEl, listType, date) {
        ...
        if (listType === "tomorrow" && !taskEl.dataset.status)
            return;
        ...
        try {
            ...
            if (btn)
                btn.textContent = completedNow ? "Undo" : (listType === "today" ? "Mark Complete" : "Cancel");
            updateProgress();
        }
        catch (err) {
    ...
    todayTasks.addEventListener("click", (e) => {
        ...
        if (target.classList.contains("complete-btn"))
            toggleTaskState(taskEl, "today", date);
    });
    ...
    addTaskButtons.forEach(btn => btn.addEventListener("click", () => {
        ...
        if (taskListSelect)
            taskListSelect.value = btn.dataset.list || "today";
    }));
    ...
    addTaskForm.addEventListener("submit", async (e) => {
        ...
        const list = listSelect ? listSelect.value : "today";
        if (!name)
            return;
        ...
        try {
            ...
            if (list === "today")
                renderTasksFromRecord(tasks, todayTasks, "today");
            else
                renderTasksFromRecord(tasks, tomorrowTasks, "tomorrow");
            updateProgress();
            addTaskForm.reset();
